...
(() => {
  ...
  class Interp {
    ...
    async exec(c) {
      switch (c.t) {
        ...
        case "switch": G.switches[c.id] = !!c.val; refreshAllPages(); evaluateQuestFailures(); break;
        ...
        case "questStart": Quests.start(c.questId); break;
        case "questAdvanceObj": Quests.advanceObjective(c.questId, c.objIndex, c.amount); evaluateQuestFailures(); break;
        case "questSetObj": Quests.setObjective(c.questId, c.objIndex, c.value); evaluateQuestFailures(); break;
        case "questComplete": {
          const res = Quests.complete(c.questId, { mapId: G.mapId, eventId: this.evRT ? this.evRT.ev.id : 0 });
          if (res && res.rewardText) await showMessage("", "You received " + res.rewardText + "!");
          break;
        }
        case "questFail": Quests.fail(c.questId); break;
    ...
    testCond(cond) {
      ...
      switch (cond.kind) {
        ...
        case "quest": return Quests.status(cond.questId) === (cond.status || "active");
  ...
  const scriptApi = {
    setSwitch(id, v) { G.switches[id] = !!v; evaluateQuestFailures(); },
    getSwitch(id) { return !!G.switches[id]; },
    setVar(id, v) { G.vars[id] = v; evaluateQuestFailures(); },
    getVar(id) { return G.vars[id] || 0; },
    addGold(n) { G.gold = clamp(G.gold + n, 0, 9999999); },
    party() { return G.party; },
    quest(id) { return Quests.get(id); },
    questStatus(id) { return Quests.status(id); },
    startQuest(id) { return Quests.start(id); },
    advanceQuestObjective(id, index, amount) { return Quests.advanceObjective(id, index, amount); },
    setQuestObjective(id, index, value) { return Quests.setObjective(id, index, value); },
    completeQuest(id) { return Quests.complete(id); },
    failQuest(id) { return Quests.fail(id); },
    abandonQuest(id) { return Quests.abandon(id); },
    state() { return G; },
  };
  ...
  async function openMenu() {
    ...
    try {
      ...
      while (true) {
        ...
        if (i < 0) break;
        idx = i;
        if (i === 0) { if (await menuJournal() === "close") return; }
        else if (i === 1) await menuItems();
        else if (i === 2) await menuSkills();
        else if (i === 3) await menuEquip();
        else if (i === 4) await menuStatus();
        else if (i === 5) await saveLoadMenu("save");
        else if (i === 6) { if (await saveLoadMenu("load")) break; }
        else if (i === 7) {
  ...
  async function saveLoadMenu(mode) {
    ...
    if (mode === "save") {
      const payload = {
        ...
        data: {
          switches: G.switches, vars: G.vars, selfSw: G.selfSw, quests: G.quests,
