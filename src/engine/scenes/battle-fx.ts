/* RPGAtlas — src/engine/scenes/battle-fx.ts
   Battle visual effects, extracted verbatim from the battle scene's closure
   in the js/engine.js monolith (Phase 1 Stage B). This is the natural seam
   inside the ~1k-line battle scene: pure presentation over the battle
   window's DOM (particle pool, bursts, floating numbers, pulses, projectile
   travel, cast flashes) with no game-state access. createBattleFx(win,
   fxLayer) closes over the same two nodes the monolith's inner functions
   closed over; every effect function body is unchanged.
...
export function createBattleFx(win: any, fxLayer: any): any {
  ...
  return { fxPoint, burst, floatText, pulse, skillKind, travel, castFx };
}
